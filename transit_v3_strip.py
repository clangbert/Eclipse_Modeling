...
def transit(tgrid, naia, star, plane, img2, specx, emspec, abspec, nx=512, ny=512, anim=False, e0=[0.5], e1=[7], root='test'):
    ...
    # Set up light curves
    lc = np.zeros([2, nfil, nt, len(e0)])  # Output light curves including absorption, 1 per band
    lcfull = np.zeros((2, nfil, nt))
    lcspec = np.zeros([2, nfil, nt, len(specx)])
    ...
    for k in tqdm.tqdm(range(naia)):  # range(naia):#
        for comp, solimg in enumerate(star.solimgarr):
            ...
            for index, t in (enumerate(tgrid)):  # tqdm.tqdm
                ...
                #print(np.shape(fluximgarr))
                if k == 0:
                    fluximgarr[comp, index] = np.sum(star_spec_img_copy, axis=-1)
                lcspec[comp, k, index] = np.sum(star_spec_img_copy, axis=(0,1))

                #print('summing', time.time()-ts)
                #return
                ts=time.time()
                # Make a movie for the first AIA image (k=0)
                if k == 0 and comp == 1:
                    # print('here')
                    lcfull[comp, k, index] = np.sum(lcspec[comp, k, index])
                    if index == int(nt / 2) or (anim is True and index % 3 == 0):
                        ...
                        if cb:
                            ...
                            cb = False
                        #print('spec', np.shape(specfromimg))
                        spechere = lcspec[0, k, index]+lcspec[1, k, index]
                        ax['C'].plot(specx, spechere, c='black')
                        ax['C'].loglog()
                        ax['D'].plot(specx, spechere/(lcspec[0, k, 0]+lcspec[1, k, 0]), c='red')
                        ax['D'].set_xscale('log')
                        if index == int(nt/2):
                    ...
                    if (anim is True and index % 3 == 0) or (anim is False and index==nt-2):
                        #print('ltc')
                        #print((lcfull[0, k, index] + lcfull[1,k,index])/(lcfull[0,k,0]+lcfull[1,k,0]))
                        ax['A'].plot(tgrid[:index],
                                   (lcfull[0, k, :index] + lcfull[1, k, :index])/((lcfull[0, k, 0] + lcfull[1, k, 0])),
                                   c='blue')
    ...
    #lc0full = lc0[0] + lc0[1]
    lcspecfull = lcspec[0] + lcspec[1]
    lcfull = np.sum(lcspecfull, axis=-1) #+ np.sum(lcspecfull, axis=-1)
    ...
    if anim is True:
        ...
        fig.savefig(f'eclipse_panes/2ComponentBetter/sclht{plane.sclht}/dense{plane.nbase/5e+10}/abund{plane.abundO}/midtransit.png')
    fig.show()
    plt.show()
    plt.close()
    ...
    return lcfull, lcspecfull, lcspec
