...
def get_spec(path, e0, e1, component=1, corkT=None, coldepth=6*10**19):
    ...
    try:
        #fffff = 'g' + 6
        spec = np.loadtxt(path+'spec.csv')

        src = np.loadtxt(path+'src.csv')
        print('loaded spec from', path)
    except:
        # Simulate a spectrum
        clean()
        ...
        cts, spec, src = get_sim_spec(coldepth=coldepth, erange=(e0[0], e1[0]), component=component, verbose=1, corkT=corkT)
        print(path)
        np.savetxt(path+'spec.csv', spec)
        np.savetxt(path+'src.csv', src)
    ...
    return spec, totcts, bandcts, src
...
def get_sim_spec(coldepth=0, expo=116450.97187241, erange=(0.245, 7), verbose=0, component=1, corkT=None, abunds=[1,1,1]):
    ...
    splot= get_source_plot() # return the data used by plot_source
    xsplot = splot.x
    ysplot = splot.y
    inphalo = (xsplot >= 0.24)
    inphahi = (xsplot <= 7.007)
    mask = np.minimum(inphalo, inphahi)
    src = np.array([xsplot[mask], ysplot[mask]])

    counts = sum(ysplot)
    # print(counts, 'vs', calc_model_sum(), 'vs', calc_source_sum(), 'vs', trapz(sim_ymod, sim_xmod))
    return counts, np.array([sim_xmod, sim_ymod]), src


def get_atm_abs(star, plane, img2, nband, e0, e1, speclen, speclensrc, delt, frnt2, root, starspec):
    # Set up elemental abundances as per from getabund.pro with the values for Grevesse et al (1992).
    nx, ny = len(img2), len(img2[0])
    clean() # clear out current Sherpa session
    specimgcts = np.array([[np.zeros_like(img2)] * 2] * nband)
    specimg = np.zeros((2, nx, ny, speclen))
    specimgsrc = np.zeros((2, nx, ny, speclensrc))
    xaxis = starspec[0][0]
    ...
    for comp, corkT in enumerate([star.corkT1, star.corkT2]):
        specabsorbfil = f'../../exoXtransit/absorb_spec_img/sclht{round(plane.sclhtRJ, 5)}/dense{plane.nbase / (1e+10)}/abund{plane.abundO}/notxs_{root}_specabsorb_{nx}x{ny}_corkT{corkT}'
        print(specabsorbfil)
        ...
        try:
            ...
            with np.load(specabsorbfil + '.npz') as data:
                ...
                specimgsrc[comp] = data['arr_1.npy']
            ...
            counts, modspec, modsrc = get_sim_spec(img2[0][0], delt[0], [e0[0], e1[0]], component=comp + 1,
                                                       corkT=corkT)
            ...
            specimgsrc[comp] += modsrc[1]
            specimgsrc[comp][frnt2.nonzero()] *= 0
            xcts = modspec[0]*1
            xsrc = modsrc[0]*1
            ...
            for x, row in tqdm.tqdm(enumerate(img2[:kx])):
                for y, coldepth in enumerate(row[:ky]):
                    if coldepth > star.dist and frnt2[x, y] == 0:
                        counts, modspec, modsrc = get_sim_spec(coldepth, delt[0], [e0[0], e1[0]], component=comp + 1,
                        ...
                        specimgsrc[comp, x, y] = modsrc[1]
                        specimgsrc[comp, nx - x - 1, y] = modsrc[1]
                        specimgsrc[comp, x, ny - y - 1] = modsrc[1]
                        specimgsrc[comp, nx - x - 1, ny - y - 1] = modsrc[1]
            ...
            np.savez_compressed(specabsorbfil, specimg[comp], specimgsrc[comp])
            for band in range(nband):
                elo, ehi = e0[band], e1[band]
                specctfil = specabsorbfil + f'elo{elo}_ehi{ehi}.csv'
    ...
    img2spec = specimg * 1
    img2src = specimgsrc * 1

    return img2new, img2spec, img2src
